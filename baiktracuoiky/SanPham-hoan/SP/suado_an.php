...
<body>
    ...
    if (isset($_GET['ID'])) {
        ...
        $sql = "SELECT * FROM tbl_doan WHERE masp = '$masp'";
    ...
    if (isset($_POST["update"])) {
        ...
        if ($_FILES["linkimg"]["size"] > 0) {
            $target_dir = "/baiktracuoiky/SanPham-hoan/SP/Do_an/"; // Đường dẫn tương đối
        ...
        $sql_update = "UPDATE tbl_doan SET tensp='$tensp', ncc='$ncc', giaban='$giaban', linkimg='$linkimg' WHERE masp='$masp'";

        if (mysqli_query($conn, $sql_update)) {
            echo "<script>alert('Sản phẩm đã được cập nhật thành công'); window.location.href='do_an.php';</script>";
        } else {
