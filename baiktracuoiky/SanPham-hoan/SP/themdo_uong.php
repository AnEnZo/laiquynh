...
<body>
    ...
    if (isset($_POST["submit"])) {
        $target_dir = "C:/xampp/htdocs/baiktracuoiky/SanPham-hoan/SP/Do_uong/";
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["linkimg"]["tmp_name"], $target_file)) {
                ...
                $linkimg = "/baiktracuoiky/SanPham-hoan/SP/Do_uong/" . basename($target_file);

                $sql = "INSERT INTO tbl_douong (masp, tensp, ncc, giaban, linkimg)
                VALUES ('$masp','$tensp','$ncc', '$giaban', '$linkimg')";

                if (mysqli_query($conn, $sql)) {
                    echo "<script>alert('Sản phẩm mới đã được thêm thành công'); window.location.href='do_uong.php';</script>";
                } else {
